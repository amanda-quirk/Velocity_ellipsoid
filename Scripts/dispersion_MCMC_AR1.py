...
for i in range(10):#len(oLOS_real)):
	...
	oR_guesses = abs(np.random.normal(loc=oR_scipy, scale= .13 * oR_guess, size=nwalkers)) #will want to vary this scale factor later!
	oPhi_guesses = oR_guesses * AR1_value 
	pos = np.vstack((oR_guesses, oPhi_guesses)).T 
	#-----------------------------------------------------------------------------------------------------------------------------
	#run MCMC
	sampler = 0
